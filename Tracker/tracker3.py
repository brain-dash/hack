...
cap = cv2.VideoCapture("911.mp4")
w = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
h = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
print(w,h)
writer = cv2.VideoWriter('writer3.avi', cv2.VideoWriter_fourcc(*'XVID'), 15.0, (w, h))
...
while (fps < 10000):
    ...
    ret, frame = cap.read()
    if fps % 3 == 0:
        if t < 19: #t < 100: #or fps % 30000 == 0 or fps != 0:
            print()
            good_new1 = good_new
            old_gray = cv2.cvtColor(old_frame, cv2.COLOR_BGR2GRAY)
            p0 = cv2.goodFeaturesToTrack(old_gray, mask=None, **feature_params)
    ...
    # Select good points
        ...
        #print(good_new)
            ...
            if flag == True:
                dddd = summa / i
            if delta < dddd*0.09 or delta > dddd*10.5:
                frame = cv2.circle(frame, (int(a), int(b)), 2, color, 2)
                #cv2.putText(frame, str(i), (int(a), int(b)), cv2.FONT_HERSHEY_SIMPLEX,
                            #0.4, color, 1)
                print (i, delta)
                t = t + 1
        print('dddddddddddddddddd', dddd)
        flag = False
