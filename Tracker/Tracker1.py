...
cap = cv2.VideoCapture("234.mp4")
w = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
h = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
writer = cv2.VideoWriter('writer1.avi', cv2.VideoWriter_fourcc(*'XVID'), 30.0, (w, h))
...
while (fps < 1000):
    ...
    ret, frame = cap.read()
    if fps % 2 == 0:
        if t < 2: #or fps % 30000 == 0 or fps != 0:
            good_new1 = good_new
            old_gray = cv2.cvtColor(old_frame, cv2.COLOR_BGR2GRAY)
            p0 = cv2.goodFeaturesToTrack(old_gray, mask=None, **feature_params)
    # Create a mask image for drawing purposes Создайте изображение маски для целей рисования
    ...
    # Select good points
        ...
        #print(good_new)
            ...
            if flag == True:
                dddd = summa / i
            if delta < dddd*0.055 or delta > dddd*25.5:
